<template>
  <div class="gpu-list">
    ...
    <el-table :data="gpuList" border>
      <el-table-column prop="fixed_id" label="ID" width="120" />
...
import { getGpuList, getGpusByServer, updateGpuConfig, getGpuMetrics, getGpuInfo } from '@/api/gpu'
...
export default {
  ...
  setup() {
    ...
    const fetchGpuList = async () => {
      ...
      try {
        ...
        gpuList.value = (res.items || []).map(item => ({
          ...
          memory_used: item.memory_used ?? 0,
          memory_total: item.memory_total ?? 0,
    ...
    const handleView = async (row) => {
      ...
      try {
        const info = await getGpuInfo(String(row.fixed_id))
        currentGpu.value = info
        const metrics = await getGpuMetrics(currentGpu.value.fixed_id)
